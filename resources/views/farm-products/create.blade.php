...
@section('header')
    ...
    <p>Enter your product details below</p>
@endsection
...
@section('content')
    <section class="product-form-section" style="margin: auto; max-width: 875px;">
        <form class="product-form" method="POST" action="{{ route('farm-products.store') }}" enctype="multipart/form-data">
            @csrf
                ...
                <div style="flex: 1;">
                    ...
                    <div class="form-group" style="margin-top:20px;">
                        ...
                        <select id="unit_of_measurement" name="unit_of_measurement" required>
                            ...
                            <option value="kg" {{ old('unit_of_measurement') == 'kg' ? 'selected' : '' }}>Kilogram (kg)
                            </option>
                            <option value="bag" {{ old('unit_of_measurement') == 'bag' ? 'selected' : '' }}>Bag</option>
                            <option value="liter" {{ old('unit_of_measurement') == 'liter' ? 'selected' : '' }}>Liter
                            </option>
                            <option value="dozen" {{ old('unit_of_measurement') == 'dozen' ? 'selected' : '' }}>Dozen
                            </option>
                        </select>
                    </div>
                    ...
                    <div style="display: flex; gap:18px; align-items:center; margin-top:20px;">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" id="selling_price" name="selling_price"
                                value="{{ old('selling_price') }}" placeholder="e.g., 300000" />
                        </div>
            ...
            <div class="form-group" style="margin-top:20px;">
                ...
                <textarea id="description" name="description" placeholder="Describe the product" rows="4" required>{{ old('description') }}</textarea>
            </div>

            <!-- Buttons -->
            <div>
                <button type="submit" name="status" value="published" class="submit-btn">Add Product</button>
